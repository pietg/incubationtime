 ...
 #upper bound for incubation time 
	...
	g<-function(s,a,b){log(1-exp(-(s/b)^a))}
	g1<-Deriv(g,"a")
	g11<-Deriv(g1,"a")
	g12<-Deriv(g1,"b")
	g2<-Deriv(g,"b")
	...
	h<-function(e,s,a,b){log(exp(-((s-e)/b)^a)-exp(-(s/b)^a))}
	h1<-Deriv(h,"a")
	h11<-Deriv(h1,"a")
	h12<-Deriv(h1,"b")
	h2<-Deriv(h,"b")
...
for (iter in 1: NumIt)
{
	...
	res1 <- nloptr(x0 = x0, eval_f = eval_f0,lb = lb0, ub = ub0, 
					eval_g_ineq = eval_g_ineq0, opts = opts,data = data)
	...
	Inverse_Fisher_matrix <- inv(Fisher_matrix)
	...
	{
		x <- grid[i]
		z1 = ((x/b)^a)*log(x/b)/exp((x/b)^a)
		z2 = -a*(1/b)^(a+1)*x^a*exp(-(x/b)^a)
	...
	for (j in 1:ngrid)
	{
		x <- grid[j]
		f1[j] <- 1-exp(-(x/c2)^c1)
		upperbound[j] <- f1[j] + 1.96*sigma[j]
		lowerbound[j] <- f1[j] - 1.96*sigma[j]
		if (f(x) < lowerbound[j]|| f(x) >  upperbound[j])
			coverage[j] = coverage[j] +1
			
		length_interval[j] <- upperbound[j]-lowerbound[j]
	}
	write(length_interval,file = "length_interval_parametric_Weibull_lognormal.txt", ncol =ngrid, append = TRUE)	
}
